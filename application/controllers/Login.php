...
class Login extends CI_Controller
{
    ...
    function isLoggedIn(){
        ...
        if( !$isLoggedIn ){
            $this->load->view(self::SLUG_PREFIX.'login');
        }
        else{
    ...
    */
    public function loginMe(){
        ...
        if($this->form_validation->run() == FALSE){ $this->index(); }
			...
			$this->login_model->lastLogin(array(
			  "uid"         => $result->uid,
    ...
    public function forgotPassword(){
        ...
        if(!isset($isLoggedIn) || $isLoggedIn != TRUE){
            $this->load->view('../cp/forgotPassword');
        }
        else{
            redirect('../cp/dashboard');
        }
    ...
    function resetPasswordConfirmUser($activation_id, $email){
        ...
        if ($is_correct == 1)
        {
            $this->load->view('../cp/newPassword', $data);
        }
        else{
            redirect('../cp/login');
        }
